                ...
                <div class="trade-section__right">
                    ...
                    <div class="trade-section__block one">
                        <div class="trade-amount">
                            <div class="trade-amount__input-group">
                                <span class="trade-amount__label">@lang('Amount') (<span class="coin-symbol">{{ strstr(@$activeCoin->symbol, '_', true) }}</span>)</span>
                                <div class="trade-amount__wrapper">
                                    <input class="trade-amount__input" name="amount" value="{{ getAmount(@$activeCoin->min_binary_trade_amount) }}" type="text">
                                </div>
                            </div>
                        ...
                        <div class="trade-duration">
                            <div class="trade-duration__toggle" data-bs-toggle="dropdown">
                                ...
                                <div class="wrapper">
                                    ...
                                    <span class="value timer-value">@php echo  convertToMinutesSeconds($durations[0]) @endphp</span>
                                </div>
                            </div>
                            <div class="dropdown-menu">
                                <ul class="trade-duration-presets">
                                    @foreach ($durations ?? [] as $duration)
                                        <li class="trade-duration-presets__item">@php echo  convertToMinutesSeconds($duration) @endphp</li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="trade-section__block two">
                        <div class="percentage-box">
                            <h6 class="percentage-box__amount" style="margin: 0; font-size: 16px; font-weight: 500; color: #d1d4dc;">
                                @lang('Your payout'): <span style="color: #26a69a; font-weight: 600;"></span>
                            </h6>
                        </div>
                    </div>
                    <div class="trade-section__block three" style="display: flex; gap: 10px;">
                        <button type="button" class="btn-new btn-new--success w-100" id="higherBtn" data-direction="higher" style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 10px 16px; font-size: 14px; font-weight: 600; border-radius: 6px; border: none; transition: all 0.2s ease; flex: 1;">
                            <i class="fas fa-arrow-up" style="font-size: 14px;"></i>
                            <span>@lang('Up')</span>
                        </button>
                        <button type="button" class="btn-new btn-new--danger w-100" id="lowerBtn" data-direction="lower" style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 10px 16px; font-size: 14px; font-weight: 600; border-radius: 6px; border: none; transition: all 0.2s ease; flex: 1;">
            ...
            <div class="terminal-header">
                <div class="container-fluid container-fluid--custom">
                    <div class="d-flex align-items-center justify-content-between">
                        <h6 class="terminal-title">@lang('History')</h6>
                        <button class="terminal-toggle" type="button"><span>@lang('Show History')</span>
                ...
                <div class="container-fluid container-fluid--custom">
                    <ul class="nav nav-tabs custom--tab-new" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#running-tab" type="button" role="tab">@lang('Running')</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#closed-tab" type="button" role="tab">@lang('Closed')</button>
                        </li>
                    </ul>
                    <div class="tab-content">
                        <div class="tab-pane active" id="running-tab" role="tabpanel" aria-labelledby="home-tab" tabindex="0">
                            <div class="table-wrapper">
                                <table class="table table-new2" id="runningTradeTable">
                                    @include('Template::partials.binary_table', ['trades' => $runningTrades])
                                </table>
                            </div>
                        </div>
                        <div class="tab-pane" id="closed-tab" role="tabpanel" aria-labelledby="profile-tab" tabindex="0">
                            <div class="table-wrapper">
                                <table class="table table-new2" id="closedTradeTable">
                                    @include('Template::partials.binary_table', ['trades' => $closedTrades])
                                </table>
                            </div>
                        </div>
                    </div>
...
@push('script-lib')
    ...
    {{-- Socket.IO for Zypher real-time updates --}}
...
@push('style')
    <style>
        ...
        /* Remove TradingView watermark/attribution */
        ...
        (function($) {
            ...
            function updateCandle(data) {
                ...
                if (!currentCandle || currentCandle.time !== currentMinuteStart) {
                    ...
                    currentCandle = {
                        time: currentMinuteStart,
                        open: data.open || currentCandle.open,
                        high: Math.max(currentCandle.high, data.high || data.close),
                        low: Math.min(currentCandle.low, data.low || data.close),
                        close: data.close
                    };
                }
            ...
            // Fallback: Update candle from price only (when OHLC not available)
            function updateCandleFromPrice(price) {
                updateCandle({
                    close: price,
                    volume: 0
                });
            ...
            function updateInvestmentLine() {
                if (!candlestickSeries || !investmentPriceLine) return;
                
                var color = direction == 'higher' ? 'green' : 'red';
                candlestickSeries.removePriceLine(investmentPriceLine);
                investmentPriceLine = candlestickSeries.createPriceLine({
                ...
                setTimeout(() => {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            // Remove investment price line from candlestick series
                            if (investmentPriceLine && candlestickSeries) {
                                candlestickSeries.removePriceLine(investmentPriceLine);
                                investmentPriceLine = null;
                            }
                            ...
                            // Show WIN or LOSE notification
                            if (response.win_status == 2) {
                                // LOSE
                                notify('error', response.notification);
                            } else if (response.win_status == 1) {
                                // WIN
            ...
            function setupDirectionHoverEffects(indicator, higherBtn, lowerBtn) {
                higherBtn.addEventListener('mouseover', () => {
                    const currentPriceY = candlestickSeries.priceToCoordinate(lastPrice);
            ...
            function showDirectionText(text, color) {
                ...
                function updatePosition() {
                    if (candlestickSeries && chart && lastPrice !== undefined) {
                        const y = candlestickSeries.priceToCoordinate(lastPrice);
            ...
            function getProfit() {
                ...
                $('.percentage-box__amount span').text(parseFloat(totalProfit).toFixed(2) + ' ' + coinSymbol);
            }
